...
def clean_folder(folder_path):
    """Organizes files in the specified folder into subfolders
    based on file type."""
    for filename in os.listdir(folder_path):
        if os.path.isfile(os.path.join(folder_path, filename)):
            ...
            if file_extension:  # Check if file has an extension
                ...
                subfolder_path = create_subfolder_if_needed(folder_path,
                                                            subfolder_name)
...
if __name__ == "__main__":
    ...
    folder_path = 'EDIT THIS WITH YOUR DESKTOP PATH'
    if os.path.isdir(folder_path):
        clean_folder(folder_path)
        print("Cleaning complete")
else:
    print("Invalid folder path. Please ensure the \
